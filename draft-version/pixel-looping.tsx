...
export default function PixelLooping(props) {
  ...
  const frequency = useMemo(() => {
    ...
    const normalizedStart =
      effectiveAmplitude > 0 ? Math.max(-1, Math.min(1, startRelativeToOscillationCenter / effectiveAmplitude)) : 0

    startAngleRef.current = !isNaN(Math.asin(normalizedStart)) ? Math.asin(normalizedStart) : 0
  ...
  return (
    ...
    >
      ...
      {RenderTarget.current() === RenderTarget.canvas && showGuides && totalAmplitude > 0 && (
        <>
          <div
            style={
              loopAxis === "y"
                ? { ...guideDotStyle("y"), top: `${guideTopPos}px` }
                : { ...guideDotStyle("x"), left: `${guideTopPos}px` }
            }
          />
          <div
            style={
              loopAxis === "y"
                ? { ...guideDotStyle("y"), top: `${guideBottomPos}px` }
                : { ...guideDotStyle("x"), left: `${guideBottomPos}px` }
            }
          />
          <div
            style={
              loopAxis === "y"
                ? { ...guideDotStyle("y"), top: `${center}px` }
                : { ...guideDotStyle("x"), left: `${center}px` }
            }
          />
        </>
      )}
